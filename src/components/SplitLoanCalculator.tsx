...
import { FormData, SplitLoanFormData, FixedPeriodLoanData } from "@/constants/loanSchema";
import { calculateLoan, calculateSplitLoan, calculateFixedPeriodLoan } from "@/lib/helper/loanCalculations";
import { LoanForm, LoanResults as LoanResultsComponent, SplitLoanForm, SplitLoanResults as SplitLoanResultsComponent, FixedPeriodForm, FixedPeriodResults } from "@/components/loan";
import { CalculatorType, LoanResults, SplitLoanResults, FixedPeriodResults as FixedPeriodResultsType } from "@/types/loan";
...
export default function SplitLoanCalculator() {
  ...
  const [fixedPeriodResults, setFixedPeriodResults] = useState<FixedPeriodResultsType | null>(null);
  ...
  const handleFixedPeriodCalculation = async (data: FixedPeriodLoanData & { futureRate?: string }) => {
    setIsCalculating(true);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <p className="text-xl text-slate-600 dark:text-slate-300 max-w-3xl mx-auto mb-8">
            Calculate payments for single loans, split loans, or fixed period loans with realistic rate change scenarios
          </p>
          ...
          <div className="flex justify-center gap-4">
            ...
            <Button
              variant={calculatorType === "fixed-period" ? "default" : "outline"}
              onClick={() => switchCalculatorType("fixed-period")}
              className="flex items-center gap-2"
            >
              <Calendar className="h-4 w-4" />
              Fixed Period
              <Badge variant="secondary" className="ml-2">Realistic</Badge>
            </Button>
          </div>
        </motion.div>
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          <div>
            ...
            {/* Fixed Period Form */}
            <div className={calculatorType === "fixed-period" ? "block" : "hidden"}>
              <FixedPeriodForm
                onSubmit={handleFixedPeriodCalculation}
                onReset={handleFixedPeriodReset}
                isCalculating={isCalculating && calculatorType === "fixed-period"}
              />
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
            <div className="p-6 rounded-lg bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm">
              ...
              <p className="text-slate-600 dark:text-slate-300 text-sm">
                Traditional loan calculation with one amount, interest rate, and term. Perfect for mortgages, personal loans, or auto financing.
              </p>
            </div>
            <div className="p-6 rounded-lg bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm">
              ...
              <p className="text-slate-600 dark:text-slate-300 text-sm">
                Break your loan into multiple tranches with different amounts, rates, and terms. Ideal for complex financing scenarios.
              </p>
            </div>
            <div className="p-6 rounded-lg bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm">
              ...
              <p className="text-slate-600 dark:text-slate-300 text-sm">
                Calculate payments for loans with known fixed rates for specific periods. Perfect for adjustable-rate mortgages and rate change planning.
              </p>
            </div>
          </div>
        </motion.div>
