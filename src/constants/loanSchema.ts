...
export const fixedPeriodLoanSchema = z.object({
  loanAmount: z.string().min(1, "Original loan amount is required").refine((val) => {
    const num = parseFloat(val);
    return !isNaN(num) && num > 0;
  }, "Original loan amount must be a positive number"),
  totalLoanTermYears: z.string().min(1, "Total loan term is required").refine((val) => {
    const num = parseFloat(val);
    return !isNaN(num) && num > 0 && num <= 50;
  }, "Total loan term must be between 1 and 50 years"),
  loanStartDate: z.string().min(1, "Loan start date is required"),
  currentBalance: z.string().min(1, "Current balance is required").refine((val) => {
    const num = parseFloat(val);
    return !isNaN(num) && num > 0;
  ...
  message: "Dates must be in correct order: Loan Start → Fixed Rate Start → Analysis Start → Analysis End → Fixed Rate End",
